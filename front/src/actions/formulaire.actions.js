...
export const createFormulaire = (formulaireData) => (dispatch) => {
  ...
  fetch('https://admin.szq.tn/api/formulaire/new', {
    method: 'POST',
...
export const getAllFormulaires = () => (dispatch) => {
    ...
    fetch('https://admin.szq.tn/api/formulaire/forms')
        .then((response) => response.json())
        .then((data) => {
            dispatch({
...
export const getFormulaire = (id) => (dispatch) => {
  ...
  fetch(`https://admin.szq.tn/api/formulaire/formulaires/${id}`, {
      method: 'GET',
  })
  .then((response) => {
      if (!response.ok) {
...
export const updateFormulaire = (id, formulaireData) => (dispatch) => {
  ...
  fetch(`https://admin.szq.tn/api/formulaire/forms/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(formulaireData),
  })
