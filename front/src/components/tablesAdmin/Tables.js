...
const Tables = () => {
  ...
  const handleAddProduct = async () => {
    ...
    try {
      const response = await fetch("https://admin.szq.tn/api/product/create", {
        method: "POST",
        body: formData,
      });
  ...
  const userColumns = [
    { field: "id", headerName: "ID",width :150 , cellClassName: "table-cell"  },
    { field: "username", headerName: "Nom",width :150  , cellClassName: "table-cell" },
    { field: "role", headerName: "Role",width :150 , cellClassName: "table-cell" },
    { field: "email", headerName: "Email",width :150 , cellClassName: "table-cell" },
    {
      field: "actions",
      headerName: "Actions",
  ...
  const productColumns = [
    { field: "name", headerName: "Nom" ,width :150 , cellClassName: "table-cell"  },
    { field: "description", headerName: "Description",width :150 , cellClassName: "table-cell"  },
    { field: "price", headerName: "Prix",width :150 , cellClassName: "table-cell" },
    { field: "type", headerName: "Type",width :150 , cellClassName: "table-cell"  },
    { field: "famille", headerName: "Famille",width :150  , cellClassName: "table-cell" },
    {
      field: "files",
      headerName: "Image",
      cellClassName: "table-cell",
      ...
      renderCell: (params) => (
        <strong>
          <IconButton
            style={{ color: "#a97a45" }}
            aria-label="view"
            onClick={() => handleProductViewOpen(params.row.realProductId)}
          >
            <VisibilityIcon />
          </IconButton>
          <IconButton
            color="info"
            aria-label="edit"
            onClick={() => handleProductEditClick(params.row.realProductId)}
          >
            <EditIcon />
          </IconButton>
          <IconButton
            color="error"
            onClick={() => handleProductDeleteConfirm(params.row.realProductId)}
          >
            <DeleteIcon />
          </IconButton>
        </strong>
  ...
  const cartColumns = [
    { field: "cartId", headerName: "Cart ID", width: 150, cellClassName: "table-cell" },
    { field: "userName", headerName: "Nom", width: 150, cellClassName: "table-cell" },
    { field: "email", headerName: "Email", width: 150, cellClassName: "table-cell" },
    { field: "phoneNumber", headerName: "Numéro", width: 150, cellClassName: "table-cell" },
    { field: "message", headerName: "Message", width: 150, cellClassName: "table-cell" },
          ...
          <div>
            <ul
              style={{
                ...
                padding: 0,
                lineHeight: "1.5",
                fontSize: '16px', // Increase the font size here
              }}
            >
              {params.value.split("\n").map((item, index) => (
                <li key={index}>{item}</li>
    ...
    },
    {
      ...
      renderCell: (params) => (
        <strong>
          <IconButton
            aria-label="view"
            onClick={() => handleCartViewOpen(params.row.cartId)}
            style={{ color: "#a97a45" }}
          >
            <VisibilityIcon style={{ color: "#a97a45" }} />
          </IconButton>
        </strong>
      ),
    },
  ...
  const cartRows = carts.map((cart) => {
    ...
    const productDetails = cart.items.reduce((details, item) => {
      const detail = `${item.product?.name || "Unknown"} (Qty: ${
        item.quantity
      })`;
...
//   };
const pieData = {
    labels: ["Produits", "Devis"],
    datasets: [
      {
        data: [products.length, carts.length],
        backgroundColor: ["#a97a45", "#474747"],
        color: "#fff",
        hoverBackgroundColor: ["#073352", "#073352"],
      },
    ],
  };
  ...
  return (
    <>
      <div>
        ...
        <h1 style={{ marginBottom: "5rem", marginTop: "5rem", color: "white" }}>
          Liste Devis 
        </h1>
        <div style={{ height: 400,width: '100%', maxWidth : '84vw', overflow:"scroll",backgroundColor: "#474747cf" }}>
          <DataGrid
            rows={cartRows}
            columns={cartColumns}
            pageSize={5}
        ...
        <div style={{ height: 400, width: "100%" }}>
          <h1
            style={{ marginBottom: "5rem", marginTop: "5rem", color: "white" }}
          >
            Liste Utilisateurs
          </h1>
          <div
            style={{ height: 400, width: '100%', maxWidth : '84vw',overflow:"scroll",backgroundColor: "#474747cf" }}
          >
            <DataGrid
              rows={userRows}
              columns={userColumns}
        ...
        <h1
          style={{ marginBottom: "3rem", marginTop: "15rem", color: "white" }}
        >
          Liste Produits
        </h1>
        <div
  ...
  <span>{loading ? "Saving..." : "Sauvegarder"}</span>
        ...
        >
          <Box sx={style}>
            ...
            <Box sx={{ mt: 4, display: "flex", justifyContent: "flex-end" }}>
              ...
              <Button
                variant="outlined"
                onClick={() => setProductConfirmOpen(false)}
                sx={{ ml: 2 }}
              >
                Annuler
              </Button>
        ...
        >
          <Box sx={style}>
            ...
            {productDetail ? (
              <Typography
                id="product-view-modal-description"
                sx={{ mt: 2, ...valueStyle }}
              >
                <span style={labelStyle}>Nom:</span> {productDetail.name}
                <br />
                <span style={labelStyle}>Description:</span>{" "}
                {productDetail.description}
                <br />
                <span style={labelStyle}>Type:</span> {productDetail.type}
                <br />
                <span style={labelStyle}>Famille:</span> {productDetail.famille}
                <br />
        ...
        >
          <Box sx={style}>
            ...
            {cartDetail ? (
              <Typography
                id="cart-view-modal-description"
                sx={{ mt: 2, ...valueStyle }}
              >
                <span style={labelStyle}>Cart ID:</span> {cartDetail.cartId}
                <br />
                <span style={labelStyle}>User Name:</span>{" "}
                {cartDetail.userInfo?.name || "Unknown"}
                <br />
                <span style={labelStyle}>Email:</span>{" "}
                {cartDetail.userInfo?.email || "Unknown"}
                <br />
                <span style={labelStyle}>Phone Number:</span>{" "}
                {cartDetail.userInfo?.phoneNumber || "Unknown"}
                <br />
                <span style={labelStyle}>Message:</span>{" "}
                {cartDetail.userInfo?.message || "Unknown"}
                <br />
                <span style={labelStyle}>Total:</span> {cartDetail.total}
                <br />
